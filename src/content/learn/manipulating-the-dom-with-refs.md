...
<Trans>React가 관리하는 DOM 노드에 접근하려면, 먼저 `useRef` 훅을 불러오세요:</Trans>
...
<Trans>그런 다음 컴포넌트 내부에서 ref를 선언하세요:</Trans>
...
<Trans>마지막으로, DOM 노드를 가져올 JSX 태그에 `ref` 속성으로 참조를 전달하세요:</Trans>
